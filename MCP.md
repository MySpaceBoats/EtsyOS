...
16 serveurs MCP, un par domaine d'integration. Aucun agent ne parle a une API externe sans passer par son
serveur MCP dedie.
...
| `image-generation` | Generation d'images multi-fournisseurs avec bascule automatique (Workers AI → Imagen → HuggingFace). | Phase 1 — implémenté |
...
| `storage` | Interface de stockage S3-compatible (Cloudflare R2). | Phase 1 — implémenté |
...
| `assets` | Gestion du cycle de vie des assets generes. | Phase 1 — partiellement implémenté (record_asset) |
...
Phase 1 : `etsy`, `printify`, `storage`, `image-generation` implementes ; `assets` partiellement
(`record_asset`). Squelettes pour les autres serveurs — voir ROADMAP.md pour l'ordre d'implementation.

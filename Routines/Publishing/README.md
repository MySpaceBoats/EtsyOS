---
type: routine
layer: routines
created: 2026-07-17
updated: 2026-07-17
status: active
---

# Publishing

## Declencheur

GitHub Actions — frequence : `daily` (voir `.github/workflows/daily.yml`).

## Frequence

`daily`

## Dependances

Agents/Publishing-Agent, Agents/Quality-Agent

## Description

Publication automatique des listings Etsy apres controle qualite.

## Logs

Chaque execution ecrit un log dans `/Logs/Publishing/YYYY-MM-DD.md`.

## Recuperation apres erreur

A definir lors de l'implementation — chaque routine doit etre idempotente et reprenable sans double-execution.

## Statut

`non-implemente` — squelette uniquement.


---
type: routine
layer: routines
created: 2026-07-17
updated: 2026-07-17
status: active
---

# Product Generation

## Declencheur

GitHub Actions — frequence : `daily` (voir `.github/workflows/daily.yml`).

## Frequence

`daily`

## Dependances

Agents/Market-Agent, Agents/Design-Agent

## Description

Generation de nouvelles idees de produits a partir des opportunites validees.

## Logs

Chaque execution ecrit un log dans `/Logs/Product-Generation/YYYY-MM-DD.md`.

## Recuperation apres erreur

A definir lors de l'implementation — chaque routine doit etre idempotente et reprenable sans double-execution.

## Statut

`non-implemente` — squelette uniquement.


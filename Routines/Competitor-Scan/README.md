---
type: routine
layer: routines
created: 2026-07-17
updated: 2026-07-17
status: active
---

# Competitor Scan

## Declencheur

GitHub Actions — frequence : `daily` (voir `.github/workflows/daily.yml`).

## Frequence

`daily`

## Dependances

Agents/Market-Agent, MCP/competitor

## Description

Analyse des boutiques et listings concurrents sur les niches suivies.

## Logs

Chaque execution ecrit un log dans `/Logs/Competitor-Scan/YYYY-MM-DD.md`.

## Recuperation apres erreur

A definir lors de l'implementation — chaque routine doit etre idempotente et reprenable sans double-execution.

## Statut

`non-implemente` — squelette uniquement.

